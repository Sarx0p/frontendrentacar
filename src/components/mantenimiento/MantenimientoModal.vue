            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-xs" :class="isDark ? 'text-gray-500' : 'text-gray-400'">
                  {{ modoEdicion ? 'Actualiza el servicio registrado' : 'Registra un mantenimiento activo para el vehículo' }}
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <div v-else class="vehicle-combobox">
                <div class="vehicle-search" :class="errors.vehiculo_id ? 'error' : ''">
                  ...
                  >
                    <i class="pi" :class="listaVehiculosAbierta ? 'pi-chevron-up' : 'pi-chevron-down'"></i>
                  </button>
                </div>

                <div v-if="vehiculoSeleccionado" class="vehicle-selected">
                  <i class="pi pi-check-circle text-xs"></i>
                  <span>{{ nombreVehiculo(vehiculoSeleccionado) }} - {{ vehiculoSeleccionado.placa }}</span>
                  <button type="button" title="Cambiar vehículo" @click="limpiarVehiculoSeleccionado">
                    <i class="pi pi-times text-xs"></i>
                  </button>
                </div>

                <div v-if="listaVehiculosAbierta" class="vehicle-options" :class="isDark ? 'dark' : ''">
...
.vehicle-combobox { display:flex; flex-direction:column; gap:0.5rem; }
.vehicle-search { display:flex; align-items:center; gap:0.5rem; border:1px solid #e5e7eb; border-radius:0.75rem; padding:0 0.55rem; min-height:2.85rem; background:#fff; }
.modal-panel-dark .vehicle-search { border-color:#4b5563; background:#1f2937; color:#f3f4f6; }
.vehicle-search.error { border-color:#f87171; }
.vehicle-search-icon { flex:0 0 auto; color:#94a3b8; font-size:0.8rem; }
.vehicle-search input { flex:1; min-width:0; height:2.7rem; border:0; outline:0; background:transparent; color:inherit; font-size:0.875rem; }
.vehicle-toggle { flex:0 0 auto; width:2rem; height:2rem; border:0; border-radius:0.6rem; display:flex; align-items:center; justify-content:center; color:#64748b; background:transparent; }
.vehicle-toggle:hover { background:#f1f5f9; }
.modal-panel-dark .vehicle-toggle:hover { background:#374151; }
.vehicle-options { border:1px solid #e5e7eb; border-radius:0.85rem; background:#fff; max-height:13.5rem; overflow-y:auto; box-shadow:0 10px 25px rgba(15, 23, 42, 0.12); padding:0.35rem; }
.vehicle-options.dark { border-color:#4b5563; background:#111827; }
.vehicle-option { width:100%; border:0; border-radius:0.65rem; padding:0.65rem 0.75rem; display:flex; align-items:center; justify-content:space-between; gap:0.75rem; text-align:left; background:transparent; color:#111827; }
.vehicle-options.dark .vehicle-option { color:#f3f4f6; }
.vehicle-option:hover, .vehicle-option.selected { background:#fef2f2; color:#922b21; }
.vehicle-options.dark .vehicle-option:hover, .vehicle-options.dark .vehicle-option.selected { background:#3b1f1b; color:#fca5a5; }
.vehicle-option.muted { justify-content:flex-start; color:#64748b; cursor:default; }
.vehicle-option-title { font-weight:700; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.vehicle-option-plate { flex:0 0 auto; font-size:0.78rem; font-weight:800; color:#64748b; }
.vehicle-selected { border:1px solid #bbf7d0; border-radius:0.75rem; padding:0.65rem 0.75rem; display:flex; align-items:center; gap:0.55rem; background:#f0fdf4; color:#166534; font-size:0.85rem; font-weight:800; }
.vehicle-selected.readonly { border-color:#e5e7eb; background:#f8fafc; color:#334155; }
.modal-panel-dark .vehicle-selected { border-color:#166534; background:#052e16; color:#bbf7d0; }
.modal-panel-dark .vehicle-selected.readonly { border-color:#4b5563; background:#1f2937; color:#e5e7eb; }
.vehicle-selected span { flex:1; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.vehicle-selected button { border:0; background:transparent; color:inherit; width:1.6rem; height:1.6rem; border-radius:0.45rem; display:flex; align-items:center; justify-content:center; }
.vehicle-selected button:hover { background:rgba(22, 101, 52, 0.12); }
