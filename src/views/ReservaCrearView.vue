<template>
  <div class="min-h-screen" :class="isDark ? 'bg-gray-950' : 'bg-gray-50'">
    ...
    <div class="flex items-center gap-3 mb-6">
      ...
      <div>
        <h1 class="text-2xl font-extrabold" :class="isDark ? 'text-gray-100' : 'text-gray-900'">Nueva reserva</h1>
        <p class="text-sm mt-0.5" :class="isDark ? 'text-gray-400' : 'text-gray-500'">Completa los datos para registrar la reserva</p>
      </div>
    </div>
...
async function consultarVehiculos() {
  ...
  vehiculoSeleccionado.value = null
  try {
    vehiculosDisponibles.value = await reservasStore.fetchVehiculosDisponibles(fechaInicio.value, fechaFin.value)
